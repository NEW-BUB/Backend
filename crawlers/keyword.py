...
def get_keyword(text, stopwords=None, count=1):
    ...
    try:
        ...
        filtered_keywords = {
            ...
            if not re.match(r'.*(다|이다|있다|한다|했다|하다|보다|한|를|될|을|이|가|의|은|는|로|까지|에|게|적|고|며|면|서|부터|인인|용|른|된|춘|하|운|나|와|할|하여|하되|써|되어|도록|하기|이기|더라도|적인|것임|하지|는데)$', k)  # 조사/어미로 끝나는 키워드 제거
